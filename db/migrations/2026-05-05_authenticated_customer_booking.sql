-- Zakazivanje termina samo za registrovane kupce.
-- Jedan kupac može imati jedan auth nalog i poseban salon_clients red po salonu.
-- Gosti/anon ne dobijaju INSERT u termini.
...
drop function if exists public.create_public_booking(uuid, uuid, uuid, text, text, timestamptz, text);
...
grant execute on function public.create_authenticated_booking(uuid, uuid, uuid, text, text, timestamptz, text) to authenticated;
